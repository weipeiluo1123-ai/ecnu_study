...
| 卡片翻转 | `.perspective` + `.flip-face` | CSS 3D 翻转，`.group:hover` 触发前后两面切换 |
...
`src/lib/markdown.ts` 提供了一个自研的 Markdown → HTML 渲染器，基于**多阶段流水线**设计：

**处理流程：**

1. **提取代码块** — 将 ` ```lang\n...\n``` ` 替换为占位符 `%%CBn%%`，防止后续正则替换污染代码内容
2. **块级元素处理** — 逐行解析，识别并渲染：
   - 标题（`#` / `##` / `###`）
   - 表格（`| col | col |` → HTML `<table>`，支持列对齐 `:---:`）
   - 任务列表（`- [ ]` / `- [x]` → 带 checkbox 的列表项）
   - 无序列表（`- item`）
   - 有序列表（`1. item`）
   - 引用块（`> text`，支持多行）
   - 分割线（`---` / `***`）
   - 未识别的文本行自动包裹 `<p>` 段落标签
3. **行内元素处理** — `processInlines()` 按优先级顺序处理：
   - 图片（`![](url)`）
   - 链接（`[text](url)`）
   - 粗体+斜体（`***text***`）
   - 粗体（`**text**`）
   ...
   - **删除线**（`~~text~~` → `<del>`）
   - 行内代码（`` `code` ``）
4. **XSS 清洗** — 剥离 `<script>`、`<iframe>`、`<object>`、`<embed>`、`<style>` 标签、`on*` 事件处理器、`javascript:` 伪协议
5. **恢复代码块** — 将占位符替换回 `<pre><code>` 结构，代码内容做 HTML 转义，显示语言标签

**新增支持的语法（v2）：**

| 语法 | 渲染结果 |
|------|---------|
| `\| col1 \| col2 \|` | HTML 表格（带边框、对齐、hover 效果） |
| `- [ ] task` | 未完成复选框 |
| `- [x] task` | 已完成复选框（绿色勾 + 删除线） |
| `~~text~~` | 删除线文本 |
...
`src/components/ui/MarkdownEditor.tsx` 是一个可复用的 Markdown 编辑器组件，供**写文章页**和**编辑文章页**共享使用：

| 功能 | 说明 |
|------|------|
| **工具栏** | 12 个快捷插入按钮（Bold/Italic/H1/H2/Quote/Code/Link/Image/UL/OL/TaskList/Table） + 撤销按钮 |
| **实时预览** | 一键切换编辑/预览模式，预览区使用 `renderMarkdown()` 实时渲染 |
| **字数统计** | 右上角显示字符数 + 词数 |
| **草稿自动保存** | 1.5 秒防抖后写入 localStorage，7 天有效期，页面刷新后自动恢复 |
| **Tab 缩进** | textarea 内 Tab 键缩进 2 空格 |
| **格式切换** | MD / TXT 一键切换，TXT 模式下纯文本等宽展示 |

**组件对外 API：**
- `loadDraft(key)` — 从 localStorage 加载草稿
- `clearDraft(key)` — 发布成功后清除草稿
- Props: `content`, `onChange`, `format`, `onFormatChange?`, `draftKey?`, `placeholder?`
