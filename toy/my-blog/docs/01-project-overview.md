...
WPL Space 是基于 **Next.js 16** 的个人空间系统，包含一个**二次元风格引导页**（根域名）和一个**赛博朋克风格技术博客**（子页面），支持深色/亮色主题切换、文章分类/标签体系、全文搜索、用户系统、评论、点赞、收藏、排行榜、监控面板等功能。
...
│   │   ├── layout.tsx        # 根布局（仅 html/body/Providers）
│   │   ├── page.tsx          # 🎌 二次元风格引导页（独立于博客）
│   │   ├── not-found.tsx     # 全局 404（二次元风格）
│   │   │
│   │   ├── (blog)/           # 📚 博客路由组（赛博朋克风格）
│   │   │   ├── layout.tsx    # 博客布局（Header/Footer/AuthProvider）
